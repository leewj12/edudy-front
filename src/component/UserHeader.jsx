...
//   return (
//     <>
//       <div className="w-full bg-[#56ffd7] text-center py-2 text-sm hidden md:block">
...
//                       마이페이지
//                     </Link>
//                     <button onClick={handleLogout} className="hover:text-[#00C59E] text-gray-500 cursor-pointer">
//                       로그아웃
//                     </button>
//                   </>
//                 ) : (
...
//               className="bg-[#192a48] hover:text-[#00C59E] text-white px-3 py-0.5 rounded text-[17px] h-[29px] flex items-center cursor-pointer"
//             >
//               신청문의
//             </button>
...
export default function UserHeader() {
  ...
  useEffect(() => {
    axios.get("/guest/lecture/list/priority")
      ...
      .catch((err) => console.error("우선순위 강의 불러오기 실패:", err));
  }, []);
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 bg-white shadow">
        <div className="px-4 md:px-[128px] py-4 relative">
          <div className="flex items-center justify-between mb-2">
            <Link to="/">
              <img src="/logo.png" alt="Edudy Logo" className="h-10" />
            </Link>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden md:flex items-center gap-4 text-sm">
                {isLoggedIn ? (
                  <>
                    {(userRole === 'ROLE_ADMIN' || userRole === 'ROLE_INSTRUCTOR') && (
                      <Link to="/admin/dashboard" className="hover:text-[#00C59E] text-gray-500">관리자 사이트</Link>
                    )}
                    <Link to="/user/mypage" className="hover:text-[#00C59E] text-gray-500">마이페이지</Link>
                    <button onClick={handleLogout} className="hover:text-[#00C59E] text-gray-500 cursor-pointer">로그아웃</button>
                  </>
                ) : (
                  <>
                    <Link to="/login" className="hover:text-[#00C59E] text-gray-500">로그인</Link>
          ...
          <nav className="hidden md:flex gap-4 font-medium justify-start items-center text-[17px] h-10 text-gray-900">
            <div className="relative group font-bold text-gray-900">
              <Link to="/lectureList" className="hover:text-[#00C59E] flex items-center cursor-pointer h-full px-2 py-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
                </svg>
                &nbsp;전체과정&nbsp;&nbsp;
            ...
            {priorityLectures.map(({ lectureId, lectureShortTitle }) => (
              <Link
                key={lectureId}
                to={`/lecture/${lectureId}`}
                className="px-2 py-1 hover:text-[#00C59E] hover:bg-gray-100 rounded h-full flex items-center whitespace-nowrap"
              >
                {lectureShortTitle}
              </Link>
            ))}
          ...
          {/* 모바일 메뉴 */}
          {menuOpen && (
            <div className="md:hidden absolute top-full left-0 w-full bg-white shadow z-50 px-4 py-4 flex flex-col gap-3 text-gray-800 text-base">
              <form onSubmit={handleSearch} className="flex items-center w-full h-[35px] border border-gray-300 rounded-full px-4 shadow-sm bg-white">
                <input
                  type="text"
                  placeholder="어떤 강의를 찾고 있나요?"
                  value={searchInput}
              ...
              {priorityLectures.map(({ lectureId, lectureShortTitle }) => (
                <Link key={lectureId} to={`/lecture/${lectureId}`} className="pl-4">- {lectureShortTitle}</Link>
              ))}
              <button
                onClick={() => setShowAskModal(true)}
                className="mt-2 py-2 bg-[#192a48] text-white text-center rounded hover:bg-[#00C59E]"
              >
                신청문의
              </button>
              ...
              {isLoggedIn ? (
                <>
                  {(userRole === 'ROLE_ADMIN' || userRole === 'ROLE_INSTRUCTOR') && (
                    <Link to="/admin/dashboard">관리자 사이트</Link>
                  )}
                  <Link to="/user/mypage">마이페이지</Link>
                  <button onClick={handleLogout} className="text-left">로그아웃</button>
                </>
              ) : (
                <>
