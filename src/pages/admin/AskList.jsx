  ...
  return (
    <div className="flex w-screen h-screen overflow-hidden min-w-[1400px]">
      ...
      <main className="flex-1 overflow-y-auto bg-white p-6">
        ...
        <form className="flex gap-4 mb-4 items-center text-sm" onSubmit={(e) => { e.preventDefault(); handleSearch(); }}>
          ...
          <input type="text" placeholder="신청자 이름" className="border border-gray-400 px-3 py-2 rounded w-64" value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)} />
        ...
        <table className="w-full text-sm border-t border-b border-gray-300 text-left">
          <thead className="bg-gray-100 border-b border-gray-300">
            <tr>
              ...
              <th className="px-2 py-2">문의내용</th>
          ...
          {/* <tbody>
            {paginated.length === 0 ? (
              ...
              paginated.map(item => (
                <tr key={item.lectureAskId} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="px-2 py-2 text-center">
                    <input
                      type="checkbox"
                      checked={selectedIds.includes(item.lectureAskId)}
                      disabled={!isSelectable(item.askStatus)}
                      onChange={() => toggleCheckbox(item.lectureAskId)}
                    />
                  </td>
                  <td className="px-2 py-2">{dayjs(item.askCreatedAt).format('YYYY-MM-DD')}</td>
                  <td className="px-2 py-2">{item.lectureTitle}</td>
                  <td className="px-2 py-2 truncate max-w-xs cursor-pointer text-gray-800 hover:underline" onClick={() => setModalItem(item)}>{item.askMemo}</td>
            ...
            paginated.map(item => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-2">{dayjs(item.askCreatedAt).format('YYYY-MM-DD')}</td>
                  <td className="px-2 py-2">{item.lectureTitle}</td>
                  <td className="px-2 py-2 truncate max-w-xs text-gray-800">
        ...
        {/* {modalItem && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md shadow">
              <h3 className="text-lg font-semibold mb-4">신청 상세</h3>
              ...
              <p className="mt-4 whitespace-pre-wrap"><strong>문의내용:</strong><br />{modalItem.askMemo}</p>
              <div className="flex justify-end mt-6">
                <button className="px-4 py-1 border rounded hover:bg-gray-100" onClick={() => setModalItem(null)}>닫기</button>
              </div>
            </div>
          </div>
        )} */}
